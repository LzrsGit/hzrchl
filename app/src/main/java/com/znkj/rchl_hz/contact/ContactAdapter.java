...
public class ContactAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private int[] mContactImg;//联系人头像数组
    ...
    public ContactAdapter(Context context, String[] contactNames, int[] contactImg) {
        mContext = context;
        mLayoutInflater = LayoutInflater.from(context);
        mContactNames = contactNames;
        mContactImg = contactImg;
    ...
    private void handleContact() {
        ...
        Random random = new Random(System.currentTimeMillis());//暂时随机分配头像

        for (int i = 0; i < mContactList.size(); i++) {

            //暂时随机分配头像
            int urlIndex = random.nextInt(mContactImg.length - 1);
            int url = mContactImg[urlIndex];
            ...
            if (!characterList.contains(character)) {
                if (character.hashCode() >= "A".hashCode() && character.hashCode() <= "Z".hashCode()) { // 是字母
                    ...
                    resultList.add(new Contact(character, ITEM_TYPE.ITEM_TYPE_CHARACTER.ordinal(),url));
                } else {
                    if (!characterList.contains("#")) {
                        characterList.add("#");
                        resultList.add(new Contact("#", ITEM_TYPE.ITEM_TYPE_CHARACTER.ordinal(),url));
                    }
                }
            }

            resultList.add(new Contact(map.get(name), ITEM_TYPE.ITEM_TYPE_CONTACT.ordinal(),url));
        }
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof CharacterHolder) {
            ...
            ((ContactHolder) holder).mImageView.setImageResource(resultList.get(position).getImgUrl());

            //添加点击事件
            final int pos = position;
            final String name = resultList.get(position).getmName();
            if( mOnItemClickListener!= null){
                holder.itemView.setOnClickListener( new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        mOnItemClickListener.onClick(pos,name);
                    }
                });
                holder. itemView.setOnLongClickListener( new View.OnLongClickListener() {
                    @Override
                    public boolean onLongClick(View v) {
                        mOnItemClickListener.onLongClick(pos,name);
                        return false;
                    }
                });
            }
    ...
    public class ContactHolder extends RecyclerView.ViewHolder {
        ...
        ContactHolder(View view) {
            ...
            mImageView = (ImageView) view.findViewById(R.id.contact_img);
        }
    }
